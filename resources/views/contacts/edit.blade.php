...
<form action="{{ route('contacts.update',$contact) }}" method="POST" class="px-4 rounded mx-auto max-w-3xl w-full  inputs space-y-6">
@csrf
@method('PUT')
<div>
...
<div class="w-1/2">
  ...
  <select
    ...
    id="organization"
  >
  <option name ="organization_id" placeholder ="SELECT "value = "{{$organization->id}}">{{$contact->organization->name}}</option>
</select>
</div>
</div>
<div class="flex space-x-4">
